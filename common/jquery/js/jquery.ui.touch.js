...
$(function ()
{
	...
	//
	$.fn.EnableTouch = function (className)
	{
		if ($.support.touch)
		{
			...
			else
			{
				...
				{
					var obj = $(this)[0];
					obj.addEventListener('touchstart', iPadTouchHandler, false);
					obj.addEventListener('touchmove', iPadTouchHandler, false);
					obj.addEventListener('touchend', iPadTouchHandler, false);
					obj.addEventListener('touchcancel', iPadTouchHandler, false);
				});
			}
		}
	}
...
// prevents a click from occuring as we want the context menu
...
function iPadTouchHandler (event)
{
	...
	if (type == "mouseup" && tapValid && first.target == lastTap)
	{
		...
		simulatedEvent = document.createEvent("MouseEvent");
		// This check avoids click being emulated on a double tap
