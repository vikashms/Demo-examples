 ...
 *
 * @param Object o - class options. Object. {textdomain : 'им�?_группы_�?ообщений', messages : {textdomain1 : {}[, textdomain2 : {}]...}}
 *
 * Usage:
 * 
 * var msgs = { Hello : 'Прев�?д', 'Hello %user' : 'Превед %user' };
...
/* function to overcome depricated $.browser */
$.browser = function () {
...
var focusFromInside = null;
function allowWindowFocus() {
   
    var iFrameBody = $(document.activeElement.contentDocument).find('body'),
        allowFocus = true,
        focusSwitchedInside = false;

    iFrameBody.find('.mathquill-editable').each(function () {
         if ($(this).hasClass('hasCursor')) {

             allowFocus = false;
             if ($.browser.msie) {
                 if (focusFromInside !== true) {
                     $(this).find('textarea').blur();
                     document.getElementsByTagName('iframe')[0].contentWindow.document.body.contentEditable = 'false';
                     focusFromInside = true;
                     focusSwitchedInside = true;
    ...
    if ($.browser.msie) {
        if (focusSwitchedInside === false) {
            focusFromInside = false;
            document.getElementsByTagName('iframe')[0].contentWindow.document.body.contentEditable = 'true';
        }
    }
    return allowFocus;    
}

//var targetElemForFocus = null;
/*******************************************/
...
function elDialogForm(o) {
	var self = this;
	var checkForEmptyTable = function () {
	    if (typeof elRTE.newTableHolderId !== "undefined") {
	        var newTableBody = document.getElementsByTagName('iframe')[0].contentDocument.getElementById(elRTE.newTableHolderId)
	        if ($(newTableBody).find('table').length === 0) {
	            newTableBody.parentNode.removeChild(newTableBody);
	        }
	    }
	}
...
elRTE = function(target, opts) {
			...
			.children('.tab').click(function(e) {
				if (!$(this).hasClass('active')) {
					...
					if ($(this).hasClass('editor')) {
						self.updateEditor();
                        var allowFocus = allowWindowFocus(); if(allowFocus)
						    self.window.focus();
						self.ui.update(true);
					} else {
						self.updateSource();
    ...
    /*ie css ajax load issue*/
//	html = '<html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />';
    html = '<html xmlns="http://www.w3.org/1999/xhtml"><head>'+
    '<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />'+
    '<link rel="stylesheet" href="../../static/vendor/mathquill/css/mathquill.css", type= "text/css" />';
	$.each(self.options.cssfiles, function() {
		html += '<link rel="stylesheet" type="text/css" href="'+ "../../static/vendor/" + this+'" />';
	});
	this.doc.open();
...
//		try { this.doc.designMode = "on"; } 
		try { this.doc.body.contentEditable = true; } /*firefox issue fix*/
		catch(e) { }
    ...
    $(self.doc.body).on('focus' , function(e){
           elRTE.targetElemForFocus = e.currentTarget;
    });
...
elRTE.prototype.dom = function(rte) {
	...
	/**
	 * Вовращает индек�? �?лемента внутри родител�?
	 *
	...
	/**
	 * Вовращает значение аттрибута в нижнем реги�?тре (ох уж �?тот IE)
	 *
	 * @param  Element n  нода
	 * @param  String  attr им�? аттрибута
	 * @return string
	 **/
	...
	/**
	 * Вовращает ближайший общий контейнер дл�? 2-х �?л-тов
	 *
	...
	/**
	 * Вовращает TRUE, е�?ли нода пу�?та�?
	 * пу�?той �?читаем ноды:
	 *  - тек�?товые �?л-ты, �?одержащие пу�?тую �?троку или тег br
	 *  - тек�?товые ноды �? пу�?той �?трокой
	 *
	...
	/**
	 * Вовращает �?ледующую �?о�?еднюю ноду (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return DOMElement
	 **/
	...
	/**
	 * Вовращает предыдующую �?о�?еднюю ноду (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return DOMElement
	 **/
	...
	/**
	 * Вовращает в�?е �?ледующие �?о�?еднии ноды (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return Array
	 **/
	...
	/**
	 * Вовращает в�?е предыдующие �?о�?еднии ноды (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return Array
	 **/
	...
	/**
	 * Вовращает в�?е �?ледующие �?о�?еднии inline ноды (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return Array
	 **/
	...
	/**
	 * Вовращает в�?е предыдующие �?о�?еднии inline ноды (не включают�?�? тек�?товые ноды не �?оздающие значимые пробелы между инлайн �?лементами)
	 *
	 * @param  DOMElement n  нода
	 * @return Array
	 **/
	...
	/**
	 * Вовращает TRUE, е�?ли нода - первый непу�?той �?л-т внутри родител�?
	 *
	 * @param  Element n  нода
	 * @return bool
	 **/
	...
	/**
	 * Вовращает TRUE, е�?ли нода - по�?ледний непу�?той �?л-т внутри родител�?
	 *
	 * @param  Element n  нода
	 * @return bool
	 **/
	...
	/**
	 * Вовращает TRUE, е�?ли нода - един�?твенный непу�?той �?л-т внутри родител�?
	 *
	 * @param  DOMElement n  нода
	 * @return bool
	 **/
	...
	/**
	 * Вовращает по�?ледний непу�?той дочерний �?л-т ноды или FALSE
	 *
	 * @param  Element n  нода
	 * @return Element
	 **/
	...
	/**
	 * Возвращает TRUE, е�?ли нода "inline" 
	 *
	 * @param  DOMElement n  нода
	 * @return bool
	 **/
	...
	/**
	 * Вовращает �?лемент(ы) отвечающие у�?лови�?м пои�?ка
	 *
	 * @param  DOMElement||Array  n       нода
	 * @param  RegExp||String     filter  фильтр у�?лови�? пои�?ка (RegExp или им�? ключа this.regExp или *)
	...
	/**
	 * Вовращает ма�?�?ив родитель�?ких �?лементов, отвечающих у�?лови�?м пои�?ка
	 *
	...
	/**
	 * Вовращает ближайший родитель�?кий �?л-т, отвечающий у�?лови�?м пои�?ка
	 *
	 * @param  DOMElement     n  нода, родител�?, которой ищем
	 * @param  RegExp||String f   фильтр у�?лови�? пои�?ка (RegExp или им�? ключа this.regExp или *)
	 * @return DOMElement
	 **/
	...
	/**
	 * Вовращает или �?аму ноду или ее ближайшего родител�?, е�?ли выполн�?ют�?�? у�?лови�? sf дл�? �?амой ноды или pf дл�? родител�?
	 *
	 * @param  DOMElement     n  нода, родител�?, которой ищем
	 * @param  RegExp||String sf   фильтр у�?лови�? дл�? �?амой ноды
	* @param  RegExp||String  pf   фильтр у�?лови�? дл�? родител�?
	 * @return DOMElement
	 **/
	...
	/**
	 * Вовращает родитель�?кую ноду - �?�?ылку
	 *
	 * @param  Element n  нода
	 * @return Element
	 **/
	...
	/**
	 * Вовращает TRUE, е�?ли нода -  anchor
	 *
	 * @param  Element n  нода
	 * @return bool
	 **/
	 ...
	 *
	 * @param  DOMElement n  оборачиваема�? нода
	 * @param  DOMElement w  нода обертка или им�? тега
	 * @return DOMElement
	 **/
	...
	/**
	 * Оборачивает в�?е �?одержимое ноды
	 *
	 * @param  DOMElement n  оборачиваема�? нода
	 * @param  DOMElement w  нода обертка или им�? тега
	 * @return DOMElement
...
elRTE.prototype.options   = {
	...
	buttons         : {
        ...
        'showMathKeyboard'    : 'Equation Editor'
	},
	panels      : {
	...
	toolbars    : {
        ...
        customizedOptions    : ['tables', 'newTools']
	},
	panelNames : {
		save      : 'Save',
		copypaste : 'Copy/Pase',
...
//    debugger
	...
	/**
	 * В�?помогательна�? функци�?
	 * Возвращает �?амого верхнего родител�?, отвечающего у�?ловию - текуща�? нода - его един�?твенна�? непу�?та�? дочерн�?�? нода
	 *
	 * @param   DOMElement  n нода, дл�? которой ищем родител�?
	 * @param   DOMElement  p е�?ли задана - нода, выше которой не поднимаем�?�?
	 * @param   String      s �?трона пои�?ка (left||right||null)
	 * @return  DOMElement
	 **/
	...
	/**
	 * Возвращает TRUE, е�?ли выделение "�?хлопнуто"
	 *
	 * @return  bool
	 **/
	 ...
	 *
	 * @param   bool  toStart  �?хлопнуть к начальной точке
	 * @return  void
	 **/
	 ...
	 * Дл�? нормальных браузеров - нативный range
	 * дл�? "�?амизнаетечего" - �?мул�?цию w3c range
	 *
...
//        debugger
	 ...
	 * Выдел�?ет ноды
	 *
	 * @param   DOMNode  s  нода начала выделени�?
	 * @param   DOMNode  e  нода конца выделени�?
	 * @return  selection
	 **/
	...
	/**
	 * В�?тавл�?ет ноду в текущее выделение
	 *
	 * @param   Element  n  нода
	 * @return  selection
	 **/
	...
	/**
	 * В�?тавл�?ет html в текущее выделение
	 *
	 * @param   Element  n  нода
	 * @return  selection
	 **/
	...
	this.getBookmark = function() {
        //return; /*temp fix*/
        var allowFocus = allowWindowFocus(); 
        if(!allowFocus){
		    return    
        }
	...
	/**
	 * Возвращает ноду начала выделени�?
	 *
	 * @return  DOMElement
	 **/
	...
	/**
	 * Возвращает ноду конца выделени�?
	 *
	 * @return  DOMElement
	 **/
	...
	/**
	 * Возвращает выбраную ноду (общий контейнер в�?ех выбранных нод)
	 *
	 * @return  Element
	 **/
	...
	/**
	 * Возвращает ма�?�?ив выбранных нод
	 *
	 * @param   Object  o  параметры получени�? и обработки выбраных нод
	 * @return  Array
	 **/
	this.selected = function(o) {
		var opts = {
			collapsed : false,  // вернуть выделение, даже е�?ли оно �?хлопнуто
		...
		function addToBuffer(n) {
			...
			if (ndx === null) {
				ndx = ret.length;
				ret.push('dummy'); // заглушка дл�? оборачиваемых �?лементов
			}
		}
		...
		if (sel.nodes.length>0) {
			...
			for (var i=0; i < sel.nodes.length; i++) {
				var n = sel.nodes[i];
					// первую и по�?л тек�?товые ноды разрезаем, е�?ли необходимо
					...
					switch (opts.wrap) {
						// оборачиваем только тек�?товые ноды �? br
						case 'text':
							...
							break;
						// оборачиваем в�?е инлайн �?лементы	
						case 'inline':
	...
	/**
	 * Возвращает ма�?�?ив выбранных нод, как е�?ть
	 *
	 * @param   bool           возвращать е�?ли выделение �?хлопнуто
	 * @param   bool           "блочное" выделение (тек�?товые ноды включают�?�? полно�?тью, не зави�?имо от offset)
	 * @return  Array
	 **/
	this.selectedRaw = function(collapsed, blocks) {
		...
		// возвращает true, е�?ли нода не тек�?това�? или выделена полно�?тью
		...
		// возвращает true, е�?ли нода пу�?та�? или в ней не выделено ни одного непробельного �?имвола
		...
		// начальна�? нода выделена полно�?тью - поднимаем�?�? наверх по левой �?тороне
...
//			this.rte.log('start text node is fully selected')
		...
		// конечна�? нода выделена полно�?тью - поднимаем�?�? наверх по правой �?тороне
...
//			this.rte.log('end text node is fully selected')
		...
		// блочное выделение - е�?ли ноды не �?лементы - поднимаем�?�? к родителю, но ниже контейнера
		if (blocks) {
		...
		// е�?ли контенер выделен полно�?тью, поднимаем�?�? наверх на�?колько можно
...
//		this.rte.log(ep)
		...
		//  е�?ли начальна�? нода не пу�?та�? - добавл�?ем ее
		if (!isEmptySelected(s, res.so, s.nodeType==3 ? s.nodeValue.length : null)) {
			res.nodes.push(s);
		}
		// поднимаем�?�? от начальной ноды до начальной точки
		var n = s;
		while (n !== sp) {
			var _n = n;
...
//			this.rte.log(n)
		...
		// поднимаем�?�? от конечной ноды до конечной точки, результат переворачиваем
		var tmp = [];
		n = e;
		while (n !== ep) {
			var _n = n;
		...
		//  е�?ли конечна�? нода не пу�?та�? и != начальной - добавл�?ем ее
		if (!isEmptySelected(e, 0, e.nodeType==3 ? res.eo : null)) {
			res.nodes.push(e);
		}
		
		if (blocks) {
			// добавл�?ем инлайн �?о�?едей �?лева
			if (this.rte.dom.isInline(s)) {
				res.nodes = this.rte.dom.toLineStart(s).concat(res.nodes);
				res.so    = res.nodes[0].nodeType == 1 ? null : 0;
			}
			// добавл�?ем инлайн �?о�?едей �?права
...
/**
 * Обновл�?ет кнопки - вызывает метод update() дл�? каждой кнопки
 *
 * @return void
 **/
...
elRTE.prototype.ui.prototype.buttons = {
	 ...
	 * реализует поведение по умолчанию и �?вл�?ет�?�? родителем дл�? других кнопок
	 *
	 * @param  elRTE  rte   объект-редактор
	 * @param  String name  название кнопки (команда и�?полн�?ема�? document.execCommand())
	 **/
...
/**
 * Обработчик нажати�? на кнопку на тулбаре. Выполнение команды или открытие окна|меню и тд
 *
 * @return void
 **/
 ...
 * @todo Подумать, что из �?того реально нужно и наве�?ти пор�?док. Возможно ча�?ть перене�?ти в ellib
 *
 * @author:    Dmitry Levashov (dio) dio@std42.ru
 * Copyright: Studio 42, http://www.std42.ru
 */
...
//		debugger
	...
	/**
	 * Обновл�?ет данные о выделенных нодах
	 *
	 * @return void
	 **/
	 ...
	 * "Схлопывает" выделение
	 *
	 * @param  bool  toStart - �?хлопывать выделение к началу или к концу
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает начaло выделени�? на указаную ноду
	 *
	 * @param  Element  node    нода
	 * @param  Number   offset  от�?туп от начала ноды
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает конец выделени�? на указаную ноду
	 *
	 * @param  Element  node    нода
	 * @param  Number   offset  от�?туп от конца ноды
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает начaло выделени�? перед указаной нодой
	 *
	 * @param  Element  node    нода
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает начaло выделени�? по�?ле указаной ноды
	 *
	 * @param  Element  node    нода
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает конец выделени�? перед указаной нодой
	 *
	 * @param  Element  node    нода
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает конец выделени�? по�?ле указаной ноды
	 *
	 * @param  Element  node    нода
	 * @return void
	 **/
	...
	/**
	 * У�?танавливает новое выделение по�?ле изменений
	 *
	 * @return void
	 **/
...
elRTE.prototype.ui.prototype.buttons.anchor = function(rte, name) {
	...
	this.command = function() {
		...
		setTimeout(function() { var allowFocus = allowWindowFocus(); if(allowFocus) self.input.focus()}, 20);
	}
...
/**
 * @class кнопка - Цитата
 * Е�?ли выделение �?хлопнуто и находит�?�? внутри цитаты - она удал�?ет�?�?
 * �?овые цитаты �?оздают�?�? только из не�?хлопнутого выделени�?
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 *
...
/**
 * @class кнопки "копировать/вырезать/в�?тавить" 
 * в firefox показывает предложение нажать Ctl+c, в о�?тальных - копирует
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 *
	...
	elRTE.prototype.ui.prototype.buttons.css = function(rte, name) {
		...
		this.command = function() {
			...
			if (n) {
				...
				d.open();
				setTimeout(function() { var allowFocus = allowWindowFocus(); if(allowFocus) self.cssStyle.focus() }, 20)
			}
		}
 ...
 * @class кнопка - DIV
 * Е�?ли выделение �?хлопнуто и находит�?�? внутри div'a - он удал�?ет�?�?
 * �?овые div'ы �?оздают�?�? только из не�?хлопнутого выделени�?
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 * 
...
/**
 * @class кнопка - Включение/выключение показа �?труктуры документа
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 * 
...
/**
 * @class Увеличение от�?тупа
 * �?пи�?ки - е�?ли выделен один �?лемент - увеличивает�?�? вложенно�?ть �?пи�?ка, в о�?тальных �?луча�?х - padding у родитель�?кого ul|ol
 * Е�?ли таблица выделена полно�?тью - ей добавл�?ет�?�? margin, е�?ли ча�?тично - увеличивает�?�? padding дл�? �?чеек
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 *
			...
			this.rte.selection.moveToBookmark(bm);
				...
				if (this.img && this.img.parentNode) {
					...
					this.link.each(function() {
						...
						// удал�?ем �?�?ылки вокруг пу�?тых �?лементов
						if (!$.trim($this.html()) && !$.trim($this.text())) {
							$this.replaceWith($this.text()); //  �?охран�?ем пробелы :)
						}
					});
				}
 ...
 * уменьшает padding/margin/�?амомнение ;)
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
...
elRTE.prototype.ui.prototype.buttons.pasteformattext = function(rte, name) {
	...
	this.command = function() {
		...
		setTimeout(function() { var allowFocus = allowWindowFocus(); if(allowFocus) self.iframe[0].contentWindow.focus(); }, 50);
	}
...
/**
 * @class кнопка "в�?тавить только тек�?т" 
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 *
 ...
 * Е�?ли выделение �?хлопнуто и находит�?�? внутри div'a �? аттрибутом или css clear - он удал�?ет�?�?
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 *
...
elRTE.prototype.ui.prototype.buttons.table = function(rte, name) {
    ...
    this.command = function() {
       ...
       elRTE.newTableHolderId = 'custom-table-holder' + parseInt(this.rte.doc.getElementsByClassName('customTable').length + 1);
       this.newTable = $(self.rte.doc.createElement('span'))[0];
       
       
       $(this.newTable).attr('id', elRTE.newTableHolderId);     
       $(this.newTable).addClass('customTable');      
       this.rte.selection.insertNode(this.newTable);
        ...
        /*Commented to hide all other options excepts rows, cols*/
        /*
        ...
        */
        ...
        for (var tab in this.src) {
            /*table simplification*/
            /*Commented to hide advanced & event tab*/
...
//                  .append([this.rte.i18n('Background'),    $('<span />').append($('<span />').css({'float' : 'left', 'margin-right' : '3px'}).append(this.src.main.bg)).append(this.src.main.bgimg)], 'main', true)
                ...
                /*table simplification*/
                /*continue added to hide advanced & event tab*/     
                continue;
                /***/
    ...
    this.set = function() {
        ...
        var self = this;
        //setTimeout(function(){             
       // if (!self.table.parents().length) {
            var r = parseInt(self.src.main.rows.val()) || 0;
            var c = parseInt(self.src.main.cols.val()) || 0;

            self.newTable = $(self.rte.doc.getElementById(elRTE.newTableHolderId));
            ...
            var renderedTable = '<table>';
           
            for (var i=0; i < r; i++) {
                renderedTable += '<tr>';
                for (var j=0; j < c; j++) {
                    renderedTable += '<td>&nbsp</td>';
                }
                renderedTable += '</tr>';
            }
             
             renderedTable += '</table>';
             self.newTable.append(renderedTable);
             self.newTable = self.newTable.find('table')
             self.newTable.attr('border' , '1');
             self.newTable.css({'width':'90%','border-collapse':'collapse', 'margin':'auto'});
             self.newTable.find('td').css({'width': 100/c + '%', 'height': '21px'});
            
             /*if(elRTE.targetElemForFocus !== null){
              elRTE.targetElemForFocus.appendChild(newTable);
              }
              else{
              self.rte.selection.insertNode(newTable);
              }*/
        //}


//       if(elRTE.targetElemForFocus !== null) {
//        elRTE.targetElemForFocus.focus();
//        }
//        else{
//        $(self.rte.window).focus();
//        }
...
//            var newTable = $(self.rte.doc.createElement('table'))[0];
//            
//            $(newTable).attr('class', 'customTable')
//            
//            for (var i=0; i < r; i++) {
//                var tr = '<tr>';
//                for (var j=0; j < c; j++) {
...
//             $(newTable).css({'width':'90%','border-collapse':'collapse', 'margin':'auto'});
//             $(newTable).find('td').css({'width': 100/c + '%', 'height': '21px'});
//            
//             if(elRTE.targetElemForFocus !== null){
//              elRTE.targetElemForFocus.appendChild(newTable);
//              }
...
//            spacing: '2px',
//            padding: '2px',
//            border:  {width: '1px', style: 'solid', color: '#000000', css: '1px solid #000000'},
//            margin:  {width: '1px', style: 'solid', color: '#000000', css: '1px solid #000000'},
//            align: '',
//            marginLeft:'5px',
//            tableMargin: 'auto'
//        };
...
//            bg: 'rgba(0, 0, 0, 0)',
//            align: '',
//            padding: {left: '', top: '', right: '', bottom: '', css: ''},
...
//        var
//			w = parseInt(tableSpecs.width) || '',
//			h = parseInt(tableSpecs.height) || '',
//			i = $.trim(tableSpecs.bgimg),
//			b = tableSpecs.border,
//			m = tableSpecs.margin,
//			f = tableSpecs.align;        
//        this.table.attr('cellspacing', tableSpecs.spacing);
//        this.table.attr('cellpadding', tableSpecs.padding);
//		
//		this.table.css({
////			width              : w ? w+this.src.main.wunit.val() : '',
////			height             : h ? h+this.src.main.hunit.val() : '',
//			width              : w ? w+tableSpecs.wunit : '',
//			height             : h ? h+tableSpecs.hunit : '',
...
//        
//        /*table simplification*/
//        tableCellProp.width = 100/c;
//        this.table.find('td').attr('width', tableCellProp.width+tableCellProp.wunit);
//        this.table.find('td').css({'border':tableCellProp.borderStyle });
//        this.table.css({ 'margin' : tableSpecs.tableMargin });
//		this.rte.ui.update();
 ...
 * @class кнопка - отмена повтор дей�?твий
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки 
 * 
...
/********************/
/**
 * @class button - my testing button link
 *
 * @param  elRTE  rte   объект-редактор
 * @param  String name  название кнопки
 *
...
(function($) {
	...
	elRTE.prototype.ui.prototype.buttons.showMathKeyboard = function(rte, name) {
		this.constructor.prototype.constructor.call(this, rte, name);
        
		this.command = function() {

            /*code to add mathquill equation editor */ 
            var newNode = $(this.rte.doc.createElement('span'))[0];
            var newHolderId = 'equationHolder' + parseInt(this.rte.doc.getElementsByClassName('equationHolder').length + 1);
            $(newNode).attr('id', newHolderId);           
            $(newNode).attr('class', 'equationHolder');
            
            this.rte.selection.insertNode(newNode);   
            
            if ($.browser.safari){
                var curBodyStructure = this.rte.doc.body.innerHTML;
                //at end
                var indexForSpace = curBodyStructure.indexOf(newHolderId) + (newHolderId + '" class="equationHolder"></span>').length;
                curBodyStructure = curBodyStructure.slice(0, indexForSpace) + '&nbsp;' + curBodyStructure.slice(indexForSpace, curBodyStructure.length);
                //at beg
                indexForSpace = curBodyStructure.indexOf(newHolderId) - ('<span id="').length;
                if(indexForSpace === 0){
                    curBodyStructure = curBodyStructure.slice(0, indexForSpace) + '&nbsp;' + curBodyStructure.slice(indexForSpace, curBodyStructure.length);            
                }
                this.rte.doc.body.innerHTML = curBodyStructure;
            }
            MathUtilities.Components.MathEditor.Keyboard.Models.Application.init($(this.rte.doc.getElementById(newHolderId)), true, false);
